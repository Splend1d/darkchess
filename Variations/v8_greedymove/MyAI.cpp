					...
					if(Referee(board,i,rowCount,color))
					{
						...
						if (last_eaten_pos >= 0) {
							if (last_eaten_pos == rowCount)
								Result -> push_back(Move2Strength { i*100+rowCount, make_tuple(0,-eaten_chess_no,self_chess_no)});
						} else {
							Result -> push_back(Move2Strength{ i*100+rowCount, make_tuple(0,-eaten_chess_no,self_chess_no) }) ;
						}
					...
					if(Referee(board,i,colCount,color))
					{
						...
						if (last_eaten_pos >= 0) {
							if (last_eaten_pos == colCount)
								Result -> push_back(Move2Strength { i*100+colCount, make_tuple(0,-eaten_chess_no,self_chess_no)});
						} else {
							Result-> push_back(Move2Strength{ i*100+colCount, make_tuple(0,-eaten_chess_no,self_chess_no) }) ;
						}
					...
					if(Move[k] >= 0 && Move[k] < 32 && Referee(board,i,Move[k],color))
					{
						if (last_eaten_pos >= 0) {
							if (last_eaten_pos == Move[k])
								Result -> push_back(Move2Strength { i*100+Move[k], make_tuple(0,-eaten_chess_no,self_chess_no)});
						} else {
...
/* generateMove Call by reference: change src,dst */
	...
	if (isInit){
		...
		//assert(false);
		t = -DBL_MAX;
		t_temp = turnx_best_score[GLOBALTURN-1];
		...
		int last_t_nodes = 1;
		while(true){
			double guess_min = t_temp - 4900;
			double guess_max = t_temp + 4900;
			//t_temp = NegaScout(this->main_chessboard, &best_move_tmp, this->Color, 0, 0, -DBL_MAX, DBL_MAX,-1,0,-1);
			fprintf(stderr,"aspire\n");
			t_temp = NegaScout(this->main_chessboard, &best_move_tmp, this->Color, 0, 0, guess_min, guess_max,-1,0,0,-1); //aspiration
			if (t_temp <= guess_min){
				fprintf(stderr,"failed low re-search\n");
				t_temp = NegaScout(this->main_chessboard, &best_move_tmp, this->Color, 0, 0, -DBL_MAX, t_temp,-1,0,0,-1);  //faied low
				
			}
			else if (t_temp >= guess_max){
				fprintf(stderr,"failed high re-search\n");
				t_temp = NegaScout(this->main_chessboard, &best_move_tmp, this->Color, 0, 0, t_temp, DBL_MAX,-1,0,0,-1);  //faied low
				
			}
			//t_temp -= OFFSET; // rescale
			if(!isTimeUp()){
				if (t_temp >= WIN || THINK_DEPTH > 28 || t_temp == DRAW_SCORE ){ 
					t = t_temp;
					...
					double projected_t_nextdepth = t_depth * time_complexity/2;
...
int getState(int* mypiece,  int* opponentpiece) {
	...
	if (mystate == 2 && opponentstate == 2){
		...
		if (nopponentpieces <= 2 and nmypieces >= 3)
...
double MyAI::Evaluate(const ChessBoard* chessboard,
	...
	if(legal_move_count == 0){ // Win, Lose
		...
		for(int i = 0; i < 32; i++){
			if(chessboard->Board[i] != CHESS_EMPTY &&
				...
				else if (p/7 == this->Color && LASTTURNBESTHASEAT == true)
		...
		if (score > 75000+OFFSET && nopponentpieces <= 2){
			...
			//fprintf(stderr, "close2preyscore %lf\n",close2preyscore);
			//assert(false);
		}
	...
	return score + piece_value + bonus + 0.5*my_extra_moves_score - opponent_extra_moves_score + close2preyscore + clusterscore + first_eat_bonus;
}
...
double MyAI::NegaScout(const ChessBoard chessboard, int* move, const int color, const int depth, const int last_move_type, double alpha, double beta, int first_eat_bonus, int acc_skips, int acc_flips, int last_eaten_pos = -1){
	...
	if (depth > max(EXCHANGE_DEPTH,THINK_DEPTH) && !(last_move_type==1) && !(last_move_type==-1)){
		this->node++;
		double val = Evaluate(&chessboard,move_count+flip_count, color,first_eat_bonus,depth) * (depth&1 ? -1 : 1);	
		// if(depth == 0 || abs(abs(val)-105717.00) < 0.01){
		// 	for (int i = 0; i < 8; ++i){
		// 		fprintf(stderr, "%d %d %d %d\n",chessboard.Board[i*4],chessboard.Board[i*4+1],chessboard.Board[i*4+2],chessboard.Board[i*4+3]);
		// 	}
		// 	assert(false);
	...
	// }
		...
		val = Evaluate(&chessboard,move_count+flip_count, color,first_eat_bonus,depth) * (depth&1 ? -1 : 1);
		...
		for (int round = 0; round < 2; ++ round){ // first round, PVPATH only, second round, others
		// search deeper
			for (vector<Move2Strength>::iterator it = Moves.begin();it != Moves.end(); ++it){  // move

				int thismove = it -> move;
				if (round == 0 && PV[THINK_DEPTH][depth] != thismove){
					continue;
				} else if (round == 1 && PV[THINK_DEPTH][depth] == thismove){
					continue;
				}
				...
				if (isDraw(&new_chessboard)==true)
					t = (OFFSET + (DRAW_SCORE)) * (depth&1 ? -1 : 1);
					...
					if (exact != -DBL_MAX){
						...
						//alpha = max(alpha,lb);
						t = -NegaScout(new_chessboard, &new_move, color^1, depth+1, move_type, -n, -std::max(alpha, m),first_eat_bonus,0,acc_flips,thismove%100);
				...
				//alpha = max(alpha, t);
				// if(abs(abs(t)-105717.00) < 0.01){
				// 	fprintf(stderr, "d:%d MOVE %d score = %.2lf\n",depth, thismove, t);
				// 	fflush(stderr);
				// }
				if(t > m){ 
					PV[THINK_DEPTH+1][depth] = thismove;
					if (n == beta  || t >= beta){ // No scouting || THINK_DEPTH - depth < 3
						...
						*move = thismove;
						...
						m = -NegaScout(new_chessboard, &new_move, color^1, depth+1, move_type, -beta, -t, first_eat_bonus,0,acc_flips); // re-search
		...
		if ((flip_count>=16|| move_count==0) && (acc_flips >= THINK_DEPTH || depth >= max(THINK_DEPTH,2))){ // If last move is only flip, check if the opponent can capture, fastforward to exchange phase	
			ChessBoard new_chessboard = chessboard;
			int move_type = MakeMoveAndReturn(&new_chessboard, -1, 0); // fake move
			//fprintf(stderr,"fakemove %d\n",depth);
			// -n, -std::max(alpha, m)
			
			t = -NegaScout(new_chessboard, &new_move, color^1,depth + 1 , -1,-DBL_MAX,DBL_MAX ,first_eat_bonus,acc_skips+1,acc_flips);
			//fprintf(stderr,"%lf\n",t);
		...
		if (flip_count && acc_flips < THINK_DEPTH && depth < max(THINK_DEPTH,2)){
			...
			for(int i = 0; i < flip_count; i++){ // flip RELAVANT positions
					...
					for(int k = 0; k < remain_count; k++){ // what pieces could covered chess be
						...
						double t = -NegaScout(new_chessboard, &new_move, color^1, depth+1, move_type, -DBL_MAX, DBL_MAX, first_eat_bonus,0,acc_flips+1);
						// if (t > 1000000){
						// 	fprintf(stderr,"%lf",t);
						// 	assert(false);
						// }
					...
					if(E_score > m){ 
						PV[THINK_DEPTH+1][depth] = Flip_Moves[i];
						m = E_score;
						*move = Flip_Moves[i];
					}
					
					
					// if(depth == 0 || abs(abs(m)-105717.00) < 0.01){
			...
			if (n_irrelavant_positions){
				...
				for(int k = 0; k < remain_count; k++){ // what pieces could covered chess be
					...
					double t = -NegaScout(new_chessboard, &new_move, color^1, depth+1, 2, -DBL_MAX, DBL_MAX, first_eat_bonus,0,acc_flips+1);
					// if (t > 1000000){
					// 	fprintf(stderr,"%lf",t);
					// 	assert(false);
					// }
